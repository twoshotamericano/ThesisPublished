...
Each of these objects has fundamental importance due to capturing different geometrical properties of the sample. 

```{corollary, name="Geometrical Properties of the Sample"}
...
We decompose each n-dimensional measurement into a component parallel and perpendicular to the unit vector, ie. $$Y_{i}=Y_{i} \bullet \frac{1}{\sqrt{n}}+d_{i}$$ where the deviance is defined as $d_{i}=Y_{i}-Y_{i} \bullet \frac{1}{\sqrt{n}}$ and $1=[1,1,..,1]^{´}$. 
...
Similar to when dealing with univariate random variables, certain "regularities" govern the sampling distribution of $\overline{X}$ and $S$. (p.196). These results are repeated without proof below:

```{lemma, name="Large Sample Properties of Sample Mean and Covariance"}
...
```{definition, name="Population Multiple Correlation Coefficient"}
...
The following theoretical arguments reinforce our understanding of the meaning of $\rho_{Y(Z)}$. They establish $\rho_{Y(Z)}^2$ as an upper limit on the correlation between Y and its linear predictor $\overline{Y}$. They establish the interpretion of $\rho_{Y(Z)}^2$ as the power of the best linear estimator to explain the variation of Y. They establish that the best linear predictor has maximum correlation with Y ie. $\rho_{Y(Z)}^2$.
...
```{lemma, name="Upper Bound on Correlation between Response Variable and Linear Predictor"}
...
```{lemma, name="Matrix identity for minimising the Mean Square Prediction Error"}
...
```{proposition, name="Link between Mean Squared Prediction Error and Multiple Correlation Coefficient"}
...
```{lemma, name="Maximising Correlation between Response Variable and Linear Predictor"}
...
In practice, we approach the problem of estimating the best linear predictor by making additional assumptions. These assumptions vary from model to model. One practical reason for this is we do not have access to the population mean $\mu$ and covariance matrix $\Sigma$, but rather their sample estimates $\overline{X}$ and $S$ respectively.
...
**Geometrical Picture of Classical Linear Regression**

In the classical linear regression model, the response is a random variable generated by a mean effect plus a random noise term.

```{definition, name="Structure of Classical Linear Regression Model"}

Let $\underset{(n \times 1)}{Y}$ denote an n-dimensional vector of response measurements, $\underset{(n \times r+1)}{X}$ the design matrix of n observations of r explanatory variables plus a constant, $\underset{(n \times 1)}\epsilon$ the n-dimensional vector of random errors. In matrix notation the model form is:
...
$$ \begin{bmatrix} Y_{1}\\
                  ...
                  Y_{N}
                  \end{bmatrix}=
                  \begin{bmatrix}1 & X_{11} & X_{12} & ...& X_{1r}\\1& X_{21} & X_{22} &... & X_{2r}\\....&....&....&....&...\\1 & X_{n1} & X_{n2} &....&X_{nr}\end{bmatrix}
                  \begin{bmatrix}\beta_{0}\\\beta_{1}\\...\\\beta_{r}\end{bmatrix}+
                  \begin{bmatrix}\epsilon_{1}\\\epsilon_{2}\\...\\\epsilon_{n}\end{bmatrix}$$


```

```{definition, name="Assumptions of Classical Linear Regression Model"}
The error terms have zero expectation:
...
$$E[Y]=X\beta=\begin{bmatrix}1 & X_{11} & X_{12} & ...& X_{1r}\\1& X_{21} & X_{22} &... & X_{2r}\\....&....&....&....&...\\1 & X_{n1} & X_{n2} &....&X_{nr}\end{bmatrix}
                  \begin{bmatrix}\beta_{0}\\\beta_{1}\\...\\\beta_{r}\end{bmatrix}$$

This can be re-written as a sum of the columns of X:

$$E[Y]=\beta_{0}\underset{(n \times 1)}{1}+ \beta_{1}\underset{(n \times 1)}{X_{1}}+...\beta_{r}\underset{(n \times 1)}{X_{r}}$$
...
```
 $$E[Y]=X\beta=\begin{bmatrix}1 & X_{11} & X_{12} & ...& X_{1r}\\1& X_{21} & X_{22} &... & X_{2r}\\....&....&....&....&...\\1 & X_{n1} & X_{n2} &....&X_{nr}\end{bmatrix}
                  \begin{bmatrix}\beta_{0}\\\beta_{1}\\...\\\beta_{r}\end{bmatrix}$$
 is comprised of a mean term and a Random Noise
