...
I want to apply Profile Analysis to statistically check that the relationships in the data are consistent with common sense. In Figure \@ref(fig:Outliers3), I segment the data according to (log) price quartile and plot the four mean co-ordinate vectors. (Each co-ordinate has been rescaled so that the units are similar.) 

In order to build an Analytics tool that accurately predicts property price based on this data set, it is reasonable to demand that there are statistically significant differences between the mean vectors in Figure \@ref(fig:Outliers3). Since we are dealing with large sample sizes, this can be achieved with a One Way Multivariate Analysis of Variance Procedure (ref \@ref(prp:largesample) ).
...
```{proposition, name="One Way Anove"}

Lets denote each 12 dimensional observation in  each price quartile as follows.

\begin{align}
Quartile 1:& X_{11},X_{12},...X_{1n_{1}}\\
Quartile 2:& X_{21},X_{22},...X_{2n_{2}}\\
Quartile 3:& X_{31},X_{32},...X_{3_{3}}\\
Quartile 4:& X_{41},X_{42},...X_{4n_{4}}
\end{align}

The key assumptions of a MANOVA Analysis become:

1  $X_{11},X_{12},...X_{1n_{g}}$ is a Random Sample of size l from a population with mean $\mu_{l}$,l=1,..,4. The Random Samples from each group are independent.

2 All populations have a common covariance matrix $\Sigma$

3 Each population is a multivariate normal

Lets denote $\mu_{i}=\mu+\tau_{i}$ where $\tau_{i}$ is the treatment effect of the ith quantile. We therefore assume the j-th observation from the ith quantile has the form:

$$X_{ij}=\mu+\tau_{i}+e_{ij}$$

where $e_{ij}$ is an independent $N(0,\sigma^2)$ univariate normal random variable.

$\square$

```

See (see @johnson2014applied, page 296, section 6.4) for further details.
...
data3<-alldata[,c("AboveGroundFloor",
               "RenovationFlag",
               "grade.x",
               "condition.x",
               "View",
